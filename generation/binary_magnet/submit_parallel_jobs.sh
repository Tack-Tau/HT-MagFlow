#!/bin/bash
# Submit multiple parallel generation jobs for binary magnets
...
if [ ! -f "$COMPOSITIONS_FILE" ]; then
    ...
    echo "Please run search_binary_magnets.py first"
    exit 1
fi
...
for i in $(seq 0 $((NUM_JOBS - 1))); do
    ...
    sed "s/gen_bin_mag_csp_%j/gen_bin_mag_csp_batch${i}_%j/" > $TEMP_SCRIPT
...
echo "Check progress: ls -d ../results/binary_csp_magnets/*_structures | wc -l"
echo "=========================================="
